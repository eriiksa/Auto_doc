...
class AutomationGUI:
    def __init__(self, root, start_callback):
        ...
        self.tivit_var = tk.BooleanVar(value=False) # Caixinha do Tivit
    ...
    def create_consulta_tab(self):
        ...
        self.tivit_check = ttk.Checkbutton(self.tab_consulta, text="Incluir busca no Tivit (processo mais lento)", variable=self.tivit_var)
        self.tivit_check.pack(pady=5, anchor="w")
    ...
    def check_initial_config(self):
        if not gerenciador_credenciais.check_all_credentials_exist():
            messagebox.showwarning("Configuração Necessária", "É a sua primeira vez usando o app ou faltam credenciais. Por favor, preencha e salve suas informações na aba 'Configurações'.")
            self.notebook.select(self.tab_config)
    
    def start_automation(self):
        use_tivit = self.tivit_var.get()
        if not gerenciador_credenciais.check_all_credentials_exist():
        ...
        threading.Thread(target=self.start_callback, args=(ctes_raw, self.stop_event, self.status_queue, creds, use_tivit)).start()
    ...
    def show_result(self, ctes_nao_encontrados):
        if not ctes_nao_encontrados or (len(ctes_nao_encontrados) == 1 and not ctes_nao_encontrados[0]):
            ...
            mensagem += "\n\nUm arquivo de texto com esta lista foi salvo na pasta 'ctes' para sua referência."
    ...
    def remove_cte_from_textarea(self, cte_to_remove):
        try:
            ...
            new_lines = [line for line in lines if line.strip() != cte_to_remove.strip()]
